...
3. Open in default browser. The command must begin with the literal `open file://` or `open http://127.0.0.1:` — quote only the part after the scheme so the permission prefix still matches paths with spaces. The quotes are not optional: zsh treats the `?` in `?v=<epoch>` as a glob and an unquoted URL fails with `no matches found` before `open` runs:
