...
#
# Plugin-owned resources (this plugin's own scripts/ and starters/) are
# referenced by skills as the literal, quoted text "${CLAUDE_PLUGIN_ROOT}"/...
# -- never as a resolved absolute path -- so this guard recognises them by
# exact text match without needing to know the plugin's real install path.
# node may execute "${CLAUDE_PLUGIN_ROOT}"/scripts/<one of the three exports>.mjs
# as its first argument, and cp may read (never write) "${CLAUDE_PLUGIN_ROOT}"/
# starters/<file>.js|.jsx as a copy source. Every other reference to
# "${CLAUDE_PLUGIN_ROOT}" anywhere else is blocked.
#
# Fails closed: any internal error blocks the command.
# Self-test: bash "${CLAUDE_PLUGIN_ROOT}"/hooks/test-guard.sh
set -euo pipefail
...
path_ok() {
  ...
  case "$t" in
    ...
    '${CLAUDE_PLUGIN_ROOT}'/*) return 1 ;;   # never a writable path here; the two legitimate reads (node's own scripts, cp's starters source) are matched and consumed before this function runs on them
    /*|~*|\$*) return 1 ;;
  esac
  [[ "$2" == READ ]] && return 0
  t="${t#PROJECT_DIR/}"
  ...
  case "$t" in
    ...
    scripts/*) return 1 ;;   # this plugin's scripts/ lives under ${CLAUDE_PLUGIN_ROOT}; a project-relative scripts/ path is never legitimate here
  esac
  return 0
}
...
while IFS= read -r seg || [[ -n "$seg" ]]; do
  ...
  # ---- cp copying a starter out of the plugin bundle: read-only source, never a destination
  if [[ "$first" == cp && "$bare" == *'${CLAUDE_PLUGIN_ROOT}/starters/'* ]]; then
    re_cp_plugin_ok='^[[:space:]]*cp[[:space:]]+(-[a-zA-Z]+[[:space:]]+)*"?\$\{CLAUDE_PLUGIN_ROOT\}"?/starters/[A-Za-z0-9_.-]+\.(js|jsx)"?[[:space:]]+([^[:space:]]+)[[:space:]]*$'
    if ! [[ "$bare" =~ $re_cp_plugin_ok ]]; then deny 'cp from the plugin bundle: only cp "${CLAUDE_PLUGIN_ROOT}"/starters/<file>.js|.jsx <in-project-destination>, nothing else on the line'; fi
    if ! path_ok "${BASH_REMATCH[3]}" cp; then deny "path argument '${BASH_REMATCH[3]}' is outside the project (or a protected file)"; fi
    continue
  fi
  ...
  if [[ "$bare" =~ $re_shell ]]; then
    re_selftest='^[[:space:]]*bash[[:space:]]+"?\$\{CLAUDE_PLUGIN_ROOT\}"?/hooks/test-guard\.sh[[:space:]]*$'
    if ! [[ "$cmd" =~ $re_selftest ]]; then deny 'shell / script interpreter (only bash "${CLAUDE_PLUGIN_ROOT}"/hooks/test-guard.sh is allowed)'; fi
  fi
  ...
  if [[ -n "$mode" ]]; then
    read -r -a toks <<< "$seg"
    start=1
    [[ "$mode" == node ]] && start=2   # token 1 is the plugin script path, already matched by re_node_ok above
    for ((ti = start; ti < ${#toks[@]}; ti++)); do
      t="${toks[$ti]}"
      [[ "$t" == -* ]] && continue
