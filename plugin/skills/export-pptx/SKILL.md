...
allowed-tools: Read Write Bash(node "${CLAUDE_PLUGIN_ROOT}"/scripts/export-pptx.mjs:*) Bash(node --version) Bash(realpath:*) Bash(mkdir -p:*) Bash(which:*) Bash(test:*) Bash(stat:*) Bash(ls:*)
---
...
2. Run the export script:
   ```
   Bash(node "${CLAUDE_PLUGIN_ROOT}"/scripts/export-pptx.mjs "<input>" "<output>")
   ```
