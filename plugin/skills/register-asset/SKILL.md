...
allowed-tools: Read Write Edit Bash(mkdir -p assets:*) Bash(cp .claude/last-preview.png assets/thumbs/:*) Bash(node "${CLAUDE_PLUGIN_ROOT}"/scripts/make-assets-index.mjs:*) Bash(realpath:*) mcp__chrome-devtools__navigate_page mcp__chrome-devtools__take_screenshot
---
...
5. **Regenerate `assets.html`:**
   - `Bash(node "${CLAUDE_PLUGIN_ROOT}"/scripts/make-assets-index.mjs)` — see script below
